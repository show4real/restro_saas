...
class AdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (Auth::user()->type == 1) {
            ...
            // revenue-CHART-END
            $transaction = array();
            $getorders = order::select("id","order_number","grand_total","order_type","payment_type","payment_id","delivery_date","delivery_time","status",DB::raw('DATE_FORMAT(created_at, "%d-%m-%Y") as created_at'))->where('vendor_id', Auth::user()->id)->whereIn('status',[1, 2])->orderByDesc('id')->get();
        }
